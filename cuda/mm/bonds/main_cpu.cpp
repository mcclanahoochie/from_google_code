...
void TestBondsCUDA() {
    ...
    // GO!
    nbonds = compute_bonds_cpu(xyz0, N, rmin, rmax, maxrad, nbins, &nblist, &bins);
    printf(" -> nbonds cpu = %d \n", nbonds);
...
void TestXyzAutoCorrCUDA() {
    float acx, axy, acz;
    int type = 1;
    compute_xyz_autocorrelation_cpu(xyz0, N, acx, axy, acz, type);
}
...
void TestBinsAutoCorrCUDA() {
    float aci;
    int type = 1;
    compute_int_autocorrelation_cpu(bins, nbins, aci, type);
}
